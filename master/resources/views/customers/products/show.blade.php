            ...
            <div class="row">
              ...
              <li><a href="#.">Add your review</a>
                        ...
                        @else
                            ...
                            @endphp
                        @endif
                        <h2 class="price marginbottom15">Rp. {{ $discountedPrice }} &nbsp;
                    ...
                    <label class="select form-control">
                      <select name="country" id="selection">
                        ...
                        <div class="cart-buttons">
                            <a class="uppercase border-radius btn-dark" href="{{ route('customers.products.addToCart', $product->slug) }}" onclick="return confirm('Are you sure?')"><i class="fa fa-shopping-basket"></i> &nbsp; Add to cart</a>
                            <a class="icons" href="#.">
    ...
    <section id="feature_product" class="bottom_half">
        <div class="container">
            <div class="row">
                ...
                <h4 class="heading uppercase bottom30">Similar products</h4>
            </div>
            @foreach ($upsells as $upsell)
                @if (!Auth::check())
                    @php
                        $discountedPrice = $upsell->price * (1 - $upsell->disc / 100);
                        $tempPrice = substr($upsell->price, 0, 1);
                        $tempDiscountedPrice = substr($discountedPrice, 0, 1);
                        $upsell->price = str_pad($tempPrice, strlen($upsell->price), 'x');
                        $discountedPrice = str_pad($tempDiscountedPrice, strlen($discountedPrice), 'x');
                    @endphp
                @else
                    @php
                        $discountedPrice = $upsell->price * (1 - $upsell->disc / 100);
                        $upsell->price = number_format($upsell->price);
                        $discountedPrice = number_format($discountedPrice);
                    @endphp
                @endif
                <div class="col-md-3 col-sm-6">
                    <div class="product_wrap bottom_half">
                        ...
                        <div class="product_desc">
                            <p class="title"><a href="{{ route('customers.products.show', $upsell->slug) }}">{{ $upsell->name }}</a></p>
                            <span class="price bottom30">Rp.{{ $discountedPrice }} &nbsp;
                                <del><span class="discount">Rp.{{ $upsell->price }}</span></del>
                            </span>
